...
- Build systems: Visual Studio/MSBuild (Windows), Xcode (macOS)

AU and AUv3 are macOS-only formats. JUCE compiles them to nothing on Windows, which is why
the Visual Studio solution has no AU project at all - it is not missing, the format simply
does not exist there. The Windows build produces VST3 only.

## Getting the source

JUCE is a **git submodule pinned at 9.0.2**, so it must be checked out along with the
project. A plain clone leaves `JUCE/` empty and every build fails on missing modules:
...
Pinning it this way removes a whole class of "works on my machine" failures. The modules
the project references are all `useLocalCopy="1"`, i.e. resolved relative to the repo
root as `JUCE/modules`, instead of a per-machine global path. Every platform therefore
builds against byte-identical JUCE sources, and a contributor does not have to install
JUCE or keep it at any particular location.
...
> has reappeared on the `ClCompile` items, and that the module paths are still
> `JUCE/modules` rather than an absolute path.

### The flatten step

Windows builds run `flatten-vst3.cmd` as a post-build step, which replaces the `.vst3`
bundle directory with the bare DLL inside it. This is a Windows-only convenience and is
scoped to the `VS2026` export in `first.jucer`; **it must never be added to the Xcode
export**, because macOS requires the bundle structure to be intact.

## Build - macOS

```sh
xcodebuild -project Builds/MacOSX/first.xcodeproj \
...
- `-arch arm64 -arch x86_64` produces a **universal binary** that runs natively on both
  Apple Silicon and Intel Macs. Dropping one of them makes the plugin invisible to half
  the machines it is installed on.
- `CODE_SIGNING_ALLOWED=NO` is for local/CI builds only. A distributable macOS plugin
  must be signed and notarised (below).

The build produces three plugin formats, which land in different output folders:

| Format | Extension | Hosts that load it |
| --- | --- | --- |
| VST3 | `.vst3` | Reaper, Cubase, Studio One, Bitwig |
| Audio Unit | `.component` | Logic Pro, GarageBand, Final Cut |
| AUv3 | `.appex` | Logic Pro, GarageBand, iOS hosts |

AU and AUv3 matter for Mac users because **Logic and GarageBand cannot load VST3 at
all**. Shipping only VST3 means the plugin does not exist for them.

### Install locations

```
~/Library/Audio/Plug-Ins/VST3/First.vst3
~/Library/Audio/Plug-Ins/Components/First.component
```

AUv3 is discovered through its containing app rather than copied by hand; run the app
that wraps the extension once so the system registers it.

### Signing and notarisation

A plugin that is not signed and notarised will be refused by Gatekeeper on any modern
macOS, and quarantine will block it on the user's machine even if it launches locally:
...
This needs a paid Apple Developer account. For local development, unsigned builds are
fine as long as Gatekeeper is not enforcing on that machine.

### macOS-specific notes

- **Display scaling needs nothing from the plugin.** JUCE component coordinates are
  logical and the host multiplies them by the display scale, so `setSize()` must **not**
  be pre-multiplied by `Display::scale`. Doing so applies the scale twice and the window
  opens far too large - which is exactly the bug this project had.
- **OpenGL is best-effort.** It is only an optimisation for the animated panel, and it is
  the least portable part of the UI. `attachTo()` returns `void`, so availability is
  checked with `isAttached()`; if no context can be created the panel falls back to the
  normal component renderer with no visual difference. This matters on macOS, where
  OpenGL is deprecated in favour of Metal but still functional.
- **No platform-specific code in `Source/`.** There are no `_WIN32` guards and no
  Windows-only APIs; the only platform branches are JUCE's own macros, and `Main.cpp`
  (which has them) is not compiled for the plugin targets at all.
