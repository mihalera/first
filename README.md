...
MSVC compiles those translation units with `/Yu"pch.h"` and stops with
`C1010: unexpected end of file while looking for precompiled header` when it does not
find that include. The JUCE module wrappers (`JuceLibraryCode/include_juce_*.cpp`)
deliberately stay out of the PCH scheme.

Every `ClCompile` item that sets `PrecompiledHeader` - either `Use` or `Create` - must
also set `<PrecompiledHeaderFile>pch.h</PrecompiledHeaderFile>`. Omitting it on the
`Create` item is what broke this build: the CL task does **not** fall back to `pch.h`,
it defaults to `stdafx.h`, so `pch.cpp` was compiled as `/Yc"stdafx.h"`, its `"pch.h"`
include never matched, the precompiled header was never created, and `PluginProcessor.cpp`
reported `C1010` while `pch.cpp` reported `C2857: '#include' statement specified with the
...
> project is re-saved from the Projucer, re-check both settings above.
